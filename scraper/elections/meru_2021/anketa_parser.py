...
# numbers nationality as Q2 — so those helpers are reused rather than restated.
...
def _repair_question_numbers(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    # Q7.1 is written without a trailing dot ("7.1 Narystė partijoje,
    # asociacijose"), which the shared strict pattern reads as question "7" —
    # colliding with the position question and dropping the membership answer.
    # Re-derive every number with the tolerant pattern instead.
    for row in rows:
        number = _parse_question_number(str(row.get("prompt", "")))
        if number:
...
def _normalize_anketa_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        # Q7.1 is answered inline, as in 2020 Seimo.
        "narystes-politinese-organizacijose": {
            "tekstas": _answer("7.1"),
        },
        "pareiskimai": {
            # Q8.x are the savivaldybių tarybų rinkimų įstatymo 36 str. 11 d.
            # declarations and Q9.x the 36 str. 12 d. conviction ones; Q10 asks
            # about the former USSR services. Keys follow the other modules
            ...
            "ar-bendradarbiavote-su-ssrs-tarnybomis": _answer("10"),
        },
