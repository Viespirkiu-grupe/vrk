...
from scraper.elections.seimo_anyksciu_panevezio_2017.sitemap import ELECTION_ID, resolve_candidate_url
...
DEFAULT_SITEMAP_PATH = Path("sitemaps/2017-balandzio-23-seimo-anyksciai-panevezys.json")
DEFAULT_SAMPLES_ROOT = Path("samples/html/2017-balandzio-23-seimo-anyksciai-panevezys")

# Candidate tabs. The private-interest tab is singular here
# ("Privačių interesų deklaracija"), unlike the mayoral elections of the same
# year, and the asset tab is plural.
EXPECTED_TABS = {
    "anketa",
    "biografija",
...
# ("savarankiskasIzdininkas_pkdId-..."), but participants whose campaign is run
# by their party have no treasurer page and that link 404s upstream. The
# participant is published as "atstovaujamasis_pkdId-..." instead.
CAMPAIGN_TREASURER_URL_PATTERN = re.compile(
    r"savarankiskas[A-Za-z]*_pkdId-(\d+)\.html$",
    flags=re.IGNORECASE,
)
...
def _label_slug_from_url(url: str) -> str:
    ...
    stem = stem.replace("savKandidatas", "").replace("Kandidatas", "")
    stem = stem.strip("-")
    return slugify(stem)


def _campaign_url_variants(url: str) -> list[str]:
    variants = [url]
    match = CAMPAIGN_TREASURER_URL_PATTERN.search(url)
    if match:
        variants.append(url[: match.start()] + f"atstovaujamasis_pkdId-{match.group(1)}.html")
    return variants


def _fetch_campaign_page(url: str) -> tuple[str, str]:
    # Returns the page and the URL it was actually served from, so the campaign
    # sub-tabs are then walked from a URL that exists.
    last_error: Exception | None = None
    for variant in _campaign_url_variants(url):
        try:
            return fetch_text(variant), variant
        except Exception as exc:
            last_error = exc
    raise last_error if last_error is not None else RuntimeError(f"Could not fetch {url}")
...
def _fetch_candidate_tabs(
    ...
    for tab in tab_links:
        ...
        if tab["url"] == entry["url"]:
            ...
            try:
                if tab["slug"] == CAMPAIGN_TAB_SLUG:
                    tab_html, resolved_tab_url = _fetch_campaign_page(tab["url"])
                else:
                    tab_html = fetch_text(tab["url"])
            except Exception as exc:
                anomalies.append(
        ...
        if tab["slug"] == CAMPAIGN_TAB_SLUG:
            campaign_tab_url = resolved_tab_url

        saved_tabs.append(
            {
                "label": tab["label"],
                "slug": file_slug,
                "url": resolved_tab_url,
                "path": str(file_path),
                "fetched": fetched,
            }
        )
