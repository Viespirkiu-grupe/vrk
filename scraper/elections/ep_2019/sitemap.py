...
ELECTION_ID = "2019-ep"
LISTING_URL = (
    "https://www.vrk.lt/kandidatai-kandidatu-sarasai-2019-ep"
    "?srcUrl=/rinkimai/904/rnk1186/kandidatai/epKandidataiPilnasSarasas.html"
)
VRK_STATINIAI_BASE = "https://www.vrk.lt/statiniai/puslapiai/"

DEFAULT_SAMPLE_PATH = Path("samples/html/2019-ep/list.html")
DEFAULT_WRAPPER_SAMPLE_PATH = Path("samples/html/2019-ep/page.html")
DEFAULT_SITEMAP_PATH = Path("sitemaps/2019-ep.json")
...
def _select_candidate_rows(soup: BeautifulSoup) -> list[Any]:
    ...
    if table is None:
        ...
        for candidate_table in soup.select("table.partydata"):
            count = len(candidate_table.select("a[href*='epKandidatasAnketa']"))
            if count > best_count:
