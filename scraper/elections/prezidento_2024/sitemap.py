...
ELECTION_ID = "2024-prezidento"
LISTING_URL = (
    "https://www.vrk.lt/kandidatai-i-respublikos-prezidentus-2024-pr"
    "?srcUrl=/rinkimai/1504/rnk1786/kandidatai/preKandidatai.html"
)
VRK_STATINIAI_BASE = "https://www.vrk.lt/statiniai/puslapiai/"

DEFAULT_SAMPLE_PATH = Path("samples/html/2024-prezidento/list.html")
DEFAULT_WRAPPER_SAMPLE_PATH = Path("samples/html/2024-prezidento/page.html")
DEFAULT_SITEMAP_PATH = Path("sitemaps/2024-prezidento.json")

# 2024 candidate anketa links use the "KandidatasAnketa" file stem (the 2019
# module used "preKandidatasAnketa"); the marker below is a substring of both.
...
def _select_candidate_rows(soup: BeautifulSoup) -> list[Any]:
    # The presidential listing renders a stats table (table1) and the candidate
    # table (table2); both carry class "partydata". Prefer table2, then fall
    # back to whichever partydata table actually holds candidate anketa links.
    table = (
...
def _select_candidate_link(row: Any) -> Any | None:
    # The name cell is the second column (the first is the candidate photo), so
    # the anketa anchor is not necessarily in cells[0]. Pick the anchor that
    # points at a candidate anketa page.
