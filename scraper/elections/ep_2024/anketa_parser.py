...
def _parse_biografija_html(html: str) -> dict[str, Any]:
    ...
    for tr in tr_nodes:
        ...
        answer = extract_answer_text(cell, [])
...
def _split_birth_value(value: str | None) -> tuple[str | None, str | None]:
    ...
    if len(parts) == 2:
        return normalize_text_value(parts[0]), normalize_text_value(parts[1])
...
def _parse_turto_ir_pajamu_html(html: str) -> dict[str, Any]:
    ...
    for table in tabnav.find_next_siblings("table"):
        ...
        for tr in table.find_all("tr"):
            ...
            answers = [extract_answer_text(cell, []) for cell in cells]
...
def _parse_privaciu_record_table(table: Tag) -> dict[str, Any]:
    ...
    for tr in table.find_all("tr"):
        ...
        if len(cells) >= 2:
            ...
            value = extract_answer_text(cells[0], [])
...
def _parse_privaciu_interesu_html(html: str) -> dict[str, Any]:
    ...
    for sibling in tabnav.next_siblings:
        ...
        if sibling.name == "h4":
            current = {"title": tag_text(sibling), "records": []}
    ...
    for section in sections:
        ...
        for record in records:
            ...
            for item in record.get("items", []):
                ...
                item_key = source_key(str(item.get("key", "")))
                if not item_key:
                    if item_value:
        ...
        section_key = source_key(section_title)
        if not section_key:
            continue
    ...
    if tabnav is not None:
        for sibling in tabnav.next_siblings:
            if isinstance(sibling, Tag):
                ...
                links.extend(extract_links(sibling))
            else:
