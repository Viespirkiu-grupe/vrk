...
ELECTION_ID = "2024-ep"
LISTING_URL = (
    "https://www.vrk.lt/kandidatai-kandidatu-sarasai-2024-ep"
    "?srcUrl=/rinkimai/1546/rnk1866/kandidatai/epKandidataiPilnasSarasas.html"
)
VRK_STATINIAI_BASE = "https://www.vrk.lt/statiniai/puslapiai/"

DEFAULT_SAMPLE_PATH = Path("samples/html/2024-ep/list.html")
DEFAULT_WRAPPER_SAMPLE_PATH = Path("samples/html/2024-ep/page.html")
DEFAULT_SITEMAP_PATH = Path("sitemaps/2024-ep.json")
...
def _select_candidate_rows(soup: BeautifulSoup) -> list[Any]:
    # The EP listing page renders a stats table (table1) and the candidate
    # table (table2). Both carry class "partydata", so prefer table2 and fall
    # back to whichever partydata table actually holds candidate anketa links.
    # Unlike 2019, the 2024 anketa links use the "KandidatasAnketa" file stem.
    table = (
    ...
    if table is None:
        ...
        for candidate_table in soup.select("table.partydata"):
            count = len(candidate_table.select("a[href*='KandidatasAnketa']"))
            if count > best_count:
