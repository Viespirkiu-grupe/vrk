...
# The five tabs every candidate publishes, matching the 2023 Kupiškis mayoral
# set (both declaration tabs pluralised).
BASE_EXPECTED_TABS = {
    "anketa",
    "biografija",
...
# The campaign tab is the one that varies, and it varies by role rather than by
# election: in this election a council candidate's campaign is run by the party
# list, so only candidates who also stand for mayor register as a campaign
# participant of their own. Measured over 60 sampled candidates: 0 of 30
# council-only pages carry the tab, 30 of 30 mayoral ones do. Expecting it
...
# — which municipality, which party list, which seat order, whether the person
# was elected — exists only on the listing pages, never on the candidate page
# itself, so it has to travel with the entry.
