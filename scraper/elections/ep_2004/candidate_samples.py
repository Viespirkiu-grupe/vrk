...
# A 2004 candidate is three static pages under rinkimai/2004/euro/kandidatai/:
# the questionnaire (`kand_anketa_l_<ID>.htm`), the free-text biography
# (`kand_biog_l_<ID>.htm`) and the income-and-asset declaration extracts
# (`kand_pajam_l_<ID>.htm`, "Pajamų ir turto deklaracijų pagrindinių
# duomenų išrašai"). The questionnaire's profile card links the other two;
# there is no tab bar, no private-interest declaration (the ID001 form
# was not yet required of EP candidates), no "Kita" page and no campaign
# finance page (the 2004 site publishes campaign finance as per-party PDF
# reports, not per candidate). The three are saved under the corpus's
# usual file names so the record's sections keep their cross-era keys.
SUBPAGE_LINK_PATTERN = re.compile(r"kand_(biog|pajam)_l_(\d+)\.htm$")
SUBPAGE_SLUGS = {
    "biog": "biografija",
...
def extract_tab_links(candidate_html: str, candidate_url: str) -> list[dict[str, str]]:
    ...
    file names match every other election's; the anketa entry carries the
    page's own URL, which the fetcher recognises and does not fetch twice.
    """
