...
ELECTION_ID = "2023-rugsejo-3-seimo-raseiniai-kedainiai"
LISTING_URL = (
    "https://www.vrk.lt/kandidatai-kandidatu-sarasai-pirm-seim-42"
    "?srcUrl=/rinkimai/1384/rnk1704/kandidatai/SeiKandidataiPilnasSarasas.html"
)
VRK_STATINIAI_BASE = "https://www.vrk.lt/statiniai/puslapiai/"

DEFAULT_SAMPLE_PATH = Path("samples/html/2023-rugsejo-3-seimo-raseiniai-kedainiai/list.html")
DEFAULT_WRAPPER_SAMPLE_PATH = Path("samples/html/2023-rugsejo-3-seimo-raseiniai-kedainiai/page.html")
DEFAULT_SITEMAP_PATH = Path("sitemaps/2023-rugsejo-3-seimo-raseiniai-kedainiai.json")
...
def clean_candidate_name(raw_name: str) -> str:
    # The listing marks the winner with a trailing "(V)" suffix.
...
def _select_candidate_rows(soup: BeautifulSoup) -> list[Any]:
    # The full-list page renders two stats tables (table1, table2) before the
    # candidate table (table3), exactly as the 2024 Seimo listing does.
    table = (
        soup.select_one("table#table3.partydata")
        or soup.select_one("table#table3")
        or soup.select_one("table.partydata")
    )
    if table is None:
        return []
