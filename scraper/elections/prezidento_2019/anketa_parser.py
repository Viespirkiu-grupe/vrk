...
def _normalize_anketa_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    q8_3_1 = find_row_by_question_number(rows, "8.3.1")
    q8_3_2 = find_row_by_question_number(rows, "8.3.2")
    q8_3_3 = find_row_by_question_number(rows, "8.3.3")
    q8_3_4 = find_row_by_question_number(rows, "8.3.4")
    ...
    return {
        ...
        "pareiskimai": {
            "ar-esate-pilietis-pagal-kilme": normalize_text_value(row_answer_text(q8_1)),
            "ar-gyvenate-lietuvoje-trejus-metus": normalize_text_value(row_answer_text(q8_2)),
            "ar-nebaigta-teismo-paskirta-bausme": normalize_text_value(row_answer_text(q8_3_1)),
            "ar-atliekate-karo-ar-statutine-tarnyba": normalize_text_value(row_answer_text(q8_3_2)),
            "ar-susijes-priesaika-uzsienio-valstybei": normalize_text_value(row_answer_text(q8_3_3)),
            "uzsienio-priesaikos-atsisakymas": normalize_text_value(row_answer_text(q8_3_4)),
            "ar-turite-kitos-valstybes-pilietybe": normalize_text_value(row_answer_text(q9_1)),
            "ar-turejote-kitos-valstybes-pilietybe": normalize_text_value(row_answer_text(q9_2)),
            "sutikimas-tikrinti-pilietybes-duomenis": normalize_text_value(row_answer_text(q9_3)),
        },
...
def _parse_patiketiniai_html(html: str) -> dict[str, Any]:
    soup = BeautifulSoup(html, "lxml")
    content = find_main_content_after_tabnav(soup)
    return {"entries": extract_non_empty_text_nodes(content)}
...
def _normalize_patiketiniai_data(payload: dict[str, Any]) -> list[str]:
    ...
    for entry in entries:
        value = normalize_text_value(entry)
