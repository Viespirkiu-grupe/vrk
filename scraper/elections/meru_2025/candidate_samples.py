...
from scraper.elections.meru_2025.sitemap import ELECTION_ID, resolve_candidate_url
...
# ("...deklaracijos"), matching the 2024 layout. The campaign tab is
# deliberately not expected: candidates struck off by Seimas resolution keep
# only these five, so requiring it would flag every one of them. It is still
        ...
        normalized_entries.append(
            {
                ...
                # Carried through to the sample index so the parser can report
                # candidates struck off by Seimas resolution.
                "candidateNote": str(raw_entry.get("candidateNote", "")).strip(),
                "url": url,
            }
        )
