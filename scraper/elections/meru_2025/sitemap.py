...
ELECTION_ID = "2025-kovo-16-meru"
LISTING_URL = (
    "https://www.vrk.lt/kandidatai-i-merus-2025"
    "?srcUrl=/rinkimai/1586/rnk1906/kandidatai/savKandidataiMerai.html"
)
VRK_STATINIAI_BASE = "https://www.vrk.lt/statiniai/puslapiai/"

DEFAULT_SAMPLE_PATH = Path("samples/html/2025-kovo-16-meru/list.html")
DEFAULT_WRAPPER_SAMPLE_PATH = Path("samples/html/2025-kovo-16-meru/page.html")
DEFAULT_SITEMAP_PATH = Path("sitemaps/2025-kovo-16-meru.json")

CANDIDATE_LINK_MARKER = "KandidatasAnketa"

# The listing appends a status note to the name of a candidate whose
# registration was revoked: "Povilas BEIŠYS(išbrauktas - Seimo nutarimu)".
...
def extract_candidate_note(raw_name: str) -> str:
    # Keep the status note that clean_candidate_name strips; it is the only
    # place the listing records that a candidate was struck off, and the
    # candidate page itself leaves that line blank.
    match = CANDIDATE_NOTE_PATTERN.search(normalize_space(raw_name))
    if match is None:
        return ""
