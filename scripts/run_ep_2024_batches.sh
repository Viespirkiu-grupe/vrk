...
SITEMAP_PATH="$ROOT_DIR/sitemaps/2024-ep.json"
OUTPUT_ROOT="$ROOT_DIR/data/2024-ep"
ANOMALIES_PATH="$OUTPUT_ROOT/anomalies.jsonl"
STATE_DIR="${STATE_DIR:-$ROOT_DIR/.run-state/ep-2024}"
...
if [[ -n "${SAMPLES_ROOT:-}" ]]; then
  ...
  SAMPLES_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/vrk-ep2024-samples.XXXXXX")"
  CLEANUP_SAMPLES=1
fi
...
build_pending_ids() {
  ...
  awk 'NF' "$ALL_IDS_PATH" | while IFS= read -r candidate_id; do
    ...
    if [[ -f "$OUTPUT_ROOT/${candidate_id}-2024-ep.json" ]]; then
      if ! grep -qx "$candidate_id" "$DONE_IDS_PATH"; then
...
fetch_candidate() {
  ...
  "$PYTHON_BIN" -m scraper fetch-candidate-samples 2024-ep \
...
parse_candidate() {
  ...
  "$PYTHON_BIN" -m scraper parse-anketa-samples 2024-ep \
    --samples-root "$SAMPLES_ROOT" \
