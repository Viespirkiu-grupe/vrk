...
**The corpus is personal data, and not only the candidates'.** Every record
names a person who stood for election, with their birth date (112,839
records), birth place (86,453), declared wealth and income (112,104 with an
income figure), declared convictions (1,630 candidacies, 1,321 with
detail), campaign finances and portrait — as VRK published them. It also
carries people who never stood: the children a candidate named on the
questionnaire (67,141 records), the spouse (59,590), the family members
and business partners of a private-interest declaration, the donors of a
campaign, and the 790 campaign treasurers and auditors VRK lists with
their phone and e-mail. [PERSONAL_DATA.md](PERSONAL_DATA.md) is the
field-by-field inventory (issue #142): whose data each of the corpus's
1,169 leaf paths holds and how sensitive it is, generated by
`scripts/pii_inventory.py` and pinned by a test that fails when a parser
emits a path the inventory does not classify. The release is a *profile*
of the archive: `build_distribution.py --profile public`, the default,
drops the treasurers' and auditors' phone and e-mail from every record and
strips the Exif/XMP/IPTC blocks from every portrait (1,215 of the archived
sidecars carry a GPS IFD, 35 with real coordinates) before writing
`vrk-corpus.sqlite`; the archive under `data/` and `samples-full/` is left
as fetched. The terms of reuse and the address for a removal request are
