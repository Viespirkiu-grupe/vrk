...
of the five largest non-municipal elections on 2026-08-18, the offline
re-parse of every election on 2026-08-29 and the portrait archive of
2026-09-02, and the caveats worth knowing before analysing it.
...
base64, and those 2,199 have been `photos/<candidateId>.<ext>` sidecars
since 2026-08-19. Every other era links the portrait as a URL on vrk.lt,
and for 25,332 records — 24,408 under `rawData.profile.photoSrc` and the
924 of the 1996–1999 Seimas archive under its `photoUrl` — that URL was all
the corpus held: the portrait existed for as long as VRK kept serving it,
which the 2004-era URL rot in the sitemap reconciliations shows it does
not (and one 2020 Seimas portrait already does not: vrk.lt answers 404).

`scripts/backfill_url_portraits.py` fetched them — every URL unique, no
portrait shared between siblings — into the retained trees, beside the
candidate's pages as `portrait.<ext>` plus a `portrait.json` naming the
URL, the fetch time and the sha256: in `samples-full/` and, for the 429
fixture candidates, `samples/html/`, whose tracked subset now carries them
(852 files, 25 MiB). Nothing wrote to `data/`. `write_candidate_record`
reads the retained portrait beside the page it parses and externalizes it
exactly as it does a base64 payload, so the records were regenerated by the
gate's own `reparse_diff.py --full --apply` — 25,332 records differed, in
exactly the three paths expected (`photoSrc`/`photoUrl` and `nuotrauka`
changed, `photoMeta` added), and the sweep after it is clean — and the gate
itself is unchanged: a re-parse from the retained tree reproduces the
sidecar because the portrait is part of what was retained. Measured:

| | |
|---|---:|
| URL-form records | 25,332 across 41 elections |
| fetched and archived | 25,294 — 5.2 GB, three hours at 0.2 s between requests |
| containers, sniffed from the bytes | 24,375 JPEG, 911 PNG, 6 BMP, 1 GIF, 1 TIFF |
| still a URL | 38 |
| sidecars now, all eras | 27,493 files, 5.5 GB |

The 38 are the portraits VRK never hosted itself: 32 cards of the 2000
Seimas election and all 5 of the 2005 Kėdainiai by-election link lrs.lt
hosts that answer 520 and 503 to every client, plus the one 2020 Seimas
image vrk.lt no longer serves. Each keeps its URL, carries a `photoMeta` of
`url`, `fetchedAt` and `error`, and has a `PortraitFetchFailed` event
(stage `fetch`) in its election's `anomalies.jsonl` — the corpus's first
fetch-stage events, which `reparse_diff.py --apply` now keeps when it
regenerates the parse-stage ones (it used to replace the whole file, and
would have turned every recorded failure back into "never tried"). The
container comes from the bytes, never the URL: the 2012 Seimas tree serves
six BMP and one TIFF portrait as `image/jpeg` under `.jpg` names, and 911
`.jpg` links serve PNG. Five Cloudflare 520/522 answers from vrk.lt were
...
Forty-eight defects were found and fixed while building the newer modules, or by
...
| the 2000 municipal family glued into the marital status | the card prints the family inside the "Šeimyninė padėtis" paragraph, and the whole run — status, spouse, children, relations — collapsed into one comma-glued `seimine-padetis` string. Split back into the 1997/2000-Seimas keys (`sutuoktinio-vardas-pavarde`, `vaiku-vardai-pavardes`, `seimos-nariai` as `{vardas, rysys}`): **7,579 of 9,879 records** carry members — 14,961 children, 6,927 spouses, 8 foster children, 7 grandchildren (issue #100) |
| the 2016-era spouse block dropped for want of a title | the page prints "Deklaruojančio asmens sutuoktinis, sugyventinis, partneris" as the table's first row rather than a heading, so the section arrived untitled and the normalizer's sekcija-N fallback dropped it whole — spouse name and workplace lost from `normalized` while `rawData` kept them. Titled from that row now (the same section the 2020-era parse titles itself): **1,459 records recovered the block** — 1,415 in 2016 Seimo, 26 in the September 2019 by-election, 11 in Anykščiai–Panevėžys 2017, 6 in Zanavykai 2018 (issue #100) |
| the 2008 declarant block missed by one word-form | 2008 says `Deklaruojantysis asmuo` where every later year says `Deklaruojantis asmuo`, so the hoist that keys on the latter dropped the whole untitled block — the declarant's name and the era's two workplace lines (`Darbovietė ir pareigos valstybinėje tarnyboje`, `Kitos darbovietės, pareigos`). Both spellings hoist now: **all 1,602 declaring 2008 Seimo records** carry the three keys (issue #100) |
| elected status existed only as Lithuanian prose for 5,659 records | the 2016–2025 layouts publish no results tree, and their winner note (`Išrinktas pagal sąrašą`, `… vienmandatėje … apygardoje II ture`, `… savivaldybėje II ture`, `Išrinktas II ture`) stayed a sentence in `profilis.pastaba`. Measured complete — 141/141 Seimas seats in 2016, 2020 and 2024, 11/11 EP seats in 2019 and 2024 — so eighteen elections now derive `kandidatavimas.{isrinktas, isrinktasKaip}` from it, `false` on the noteless records being a real statement. The 2019 September by-election, Zanavykai and the 2019 presidential race needed their retained trees completed first — 42 candidates re-fetched from VRK's still-live archive, and the fresh pages parsed identical to the stored records apart from the new keys (issue #100) |
| the office line above the profile card captured nowhere | every 2017–2025 page opens with one `<h4 class="h4apgKom">` — `Kandidatas į savivaldybės tarybos narius ir merus`, and on the 2024 Seimo pages each candidate's own `(dokumentai pateikti 2024-07-29)` — and no parser read it, so the office sought (the one per-candidate role signal in the 2019/2023 municipal generals) and the submission date reached neither `rawData` nor `normalized`. Captured as `profile.officeHeading` and split into `profilis.kandidatuoja-i` + `profilis.dokumentu-pateikimo-data`: **29,908 records** across fifteen elections (issue #100) |
...
- 38 portraits are links to hosts that no longer serve them: 32 cards of
  the 2000 Seimas election and all 5 of the 2005 Kėdainiai by-election
  point at `www.lrs.lt` / `www3.lrs.lt` images that answer 520 and 503 to
  every client (the other 700 cards of 2000 Seimas link vrk.lt and were
  archived), and one 2020 Seimas portrait is a 404 on vrk.lt itself. The
  records keep the URL, say in `photoMeta` when it was tried and how it
  failed, and carry a `PortraitFetchFailed` anomaly;
  `scripts/backfill_url_portraits.py --retry-failed` is the way back should
  a host return.
